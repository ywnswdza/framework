package org.losy.common.mybatis.support;
...
import org.losy.common.mybatis.support.dialect.Dialect;
import org.losy.common.mybatis.support.dialect.MySQLDialect;
...
@Intercepts( { @Signature(type = Executor.class, method = "query", args = {
		...
		ResultHandler.class }) })
public class OffsetLimitInterceptor implements Interceptor,LoggerSupport {
	public static final String MYBATIS_AUTO_COUNT_SUFFIX = "AutoCount";
	...
	void processIntercept(Invocation invocation, final Object[] queryArgs) {
		...
		if (dialect.supportsLimit() && (offset != RowBounds.NO_ROW_OFFSET || limit != RowBounds.NO_ROW_LIMIT)) {
			...
			String sql = defaultSql;			// 根据分页方言处理器生成分页语句
			...
			try {
				synchronized (rw) {
					String autoCountMsId = ms.getId() + MYBATIS_AUTO_COUNT_SUFFIX;
						...
						for (SqlNode sqlNode : list) {
							if(isSelect == false) {
								...
								if(textUp.contains("SELECT") && textUp.contains("FROM")) {
									SqlNode sqlNode2  = new TextSqlNode("SELECT count(*) as totalCount " + text.substring(textUp.indexOf("FROM")));
									list2.add(sqlNode2);
									isSelect = true;
									continue;
								}
				...
				logger.debug("Create Auto count statement "+ ms.getId() +" error.",e);
			} finally {
//				rw.readLock().unlock();
			}
		}
	...
	private MappedStatement copyFromMappedStatementCount(String newId, MappedStatement ms, SqlSource newSqlSource) {
		Builder builder = new Builder(ms.getConfiguration(), newId, newSqlSource, ms.getSqlCommandType());
